...
SELECT DISTINCT ON (A.id) A.titulo, AU.*
FROM Articulo A
JOIN Autor_Articulo AA ON A.id = AA.idArticulo
JOIN Autor AU ON AA.rutAutor = AU.rut
WHERE A.titulo ILIKE '%Software%'
ORDER BY A.id, AU.rut;
